...
def retry_with_backoff(
    ...
    Args:
        ...
        exceptions: Tuple of exception types to catch and retry (default: NetworkError, RateLimitError)
        on_retry: Optional callback function called before each retry with (exception, attempt_number)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error(
                            f"All {max_retries} attempts failed for {func_name}: {e}"
                        )
...
def retry_on_rate_limit(
    ...
    def on_retry(exception: Exception, attempt: int) -> None:
        ...
        if isinstance(exception, RateLimitError) and exception.retry_after:
            ...
            logger.info(
                f"Rate limit hit. API suggests waiting {exception.retry_after}s"
            )
...
class RetryableOperation:
    ...
    def __exit__(
        ...
        logger.warning(
            f"Attempt {self.attempt + 1} failed: {exc_val}. "
            f"Retrying in {delay:.2f}s..."
        )
        time.sleep(delay)
        self.attempt += 1
