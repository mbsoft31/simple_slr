...
class ProviderConfig(BaseModel):
    ...
    enabled: bool = True
    rate_limit: float = Field(default=1.0, gt=0, description="Requests per second")
    ...
    def validate_rate_limit(cls, v: float) -> float:
        ...
        if v > 100:
            raise ValueError("rate_limit should not exceed 100 requests/second")
        return v
...
class ProvidersConfig(BaseModel):
    ...
    openalex: ProviderConfig = Field(default_factory=lambda: ProviderConfig(enabled=True, rate_limit=5.0))
    crossref: ProviderConfig = Field(default_factory=lambda: ProviderConfig(enabled=True, rate_limit=1.0))
    arxiv: ProviderConfig = Field(default_factory=lambda: ProviderConfig(enabled=True, rate_limit=0.5))
    semantic_scholar: ProviderConfig = Field(
        default_factory=lambda: ProviderConfig(enabled=False, rate_limit=1.0),
        alias="s2"
    )
...
class DeduplicationConfig(BaseModel):
    ...
    strategy: DeduplicationStrategy = Field(
        default=DeduplicationStrategy.CONSERVATIVE,
    ...
    max_year_gap: int = Field(
        default=1,
        ge=0,
        description="Maximum year difference for duplicates"
    )
    ...
    embedding_model: str = Field(
        ...
        description="Embedding model for semantic deduplication"
    )
    use_embeddings: bool = Field(
        default=False,
        description="Enable semantic embeddings (requires additional dependencies)"
    )
...
class ClassificationConfig(BaseModel):
    ...
    method: ClassificationMethod = Field(
        default=ClassificationMethod.HEURISTIC,
...
class OutputConfig(BaseModel):
    ...
    format: str = Field(
        default="csv",
        description="Output format (csv, jsonl, both)"
    )
    include_raw: bool = Field(
        default=False,
        description="Include raw provider response in output"
    )
...
class SLRConfig(BaseModel):
    ...
    mailto: Optional[str] = Field(
        default=None,
        description="Email for polite API crawling"
    )
